...
import com.umbc.aos.multiply.Utils;
import com.umbc.aos.ws.WebServiceClient;


@WebService(endpointInterface = "com.umbc.aos.multiply.MultiplicationInterface")
public class MultiplicationImpl implements MultiplicationInterface{
	static {
		try {
			...
			for(String ip:registries) {
				...
				if(isRegistryReachable(ip.split(":")[0], ip.split(":")[1])) {
	            	WebServiceClient.addServicetoRegistry(registry, "multiply");
	            	break;
				}
				else {
					count++;
