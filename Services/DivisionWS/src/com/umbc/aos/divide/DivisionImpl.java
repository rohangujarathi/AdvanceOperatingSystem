...
import com.umbc.aos.divide.Utils;
import com.umbc.aos.ws.WebServiceClient;


@WebService(endpointInterface = "com.umbc.aos.divide.DivisionInterface")
public class DivisionImpl implements DivisionInterface{
	static {
		try {
			...
			for(String ip:registries) {
				...
				if(isRegistryReachable(ip.split(":")[0], ip.split(":")[1])) {
	            	WebServiceClient.addServicetoRegistry(registry, "divide");
	            	break;
				}
				else {
					count++;
